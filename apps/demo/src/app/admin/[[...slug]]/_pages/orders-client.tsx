...
import { Badge, Card, PageHeader, TwoCol } from './_shared';
...
export function OrderDetailClient({ order: o }: { order: MockOrder }) {
  ...
  const handleFulfill = useCallback(async () => {
    ...
    try {
      ...
      toast('Could not fulfill order (demo mode)', 'error');
    } finally {
      setFulfilling(false);
  ...
  const handleRefund = useCallback(async () => {
    ...
    try {
      ...
      toast('Could not process refund (demo mode)', 'error');
    } finally {
      setRefunding(false);
      setRefundOpen(false);
    }
  ...
  return (
    <div>
      ...
      <ConfirmDialog
        open={fulfillOpen}
        title="Fulfill order"
        message={`Mark all items in order #${o.num} as fulfilled? This will trigger shipping notifications.`}
      ...
      <ConfirmDialog
        open={refundOpen}
        title="Refund order"
        message={`Process a full refund of ${formatMoney(o.total)} for order #${o.num}? This action cannot be undone.`}
        confirmLabel="Refund"
        variant="danger"
