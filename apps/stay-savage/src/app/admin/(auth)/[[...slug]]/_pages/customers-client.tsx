...
export function CustomerDetailClient({ customerId }: { customerId: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/ecom/admin/customers/${customerId}`);
        if (!res.ok) throw new Error('API error');
        const data = await res.json();
        if (cancelled) return;
        ...
        else if (c?.email) setOrders(ORDERS.filter((o) => o.email === c.email));
      } catch {
        if (!cancelled) {
          const fallback = CUSTOMERS.find((x) => x.id === customerId);
          setCustomer(fallback ?? null);
          if (fallback) setOrders(ORDERS.filter((o) => o.email === fallback.email));
        }
      } finally {
  ...
  if (!customer) return <div className="admin-empty"><div className="admin-empty-title">Customer not found</div><a href="/admin/customers" className="admin-btn admin-btn--primary">Back to Customers</a></div>;
