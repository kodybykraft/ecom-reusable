...
export function ReturnDetailClient({ returnId }: { returnId: string }) {
  ...
  const [ret, setRet] = useState<MockReturn | null>(null);
  const [loading, setLoading] = useState(true);
  const [currentStatus, setCurrentStatus] = useState<string>('requested');
  const [timeline, setTimeline] = useState<MockReturn['timeline']>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/ecom/admin/returns/${returnId}`);
        if (!res.ok) throw new Error('API error');
        const data = await res.json();
        if (cancelled) return;
        const r = (data.data ?? data) as MockReturn;
        setRet(r);
        setCurrentStatus(r.status);
        setTimeline(r.timeline ?? []);
      } catch {
        if (!cancelled) {
          const fallback = RETURNS.find((x) => x.id === returnId) ?? RETURNS[0];
          setRet(fallback);
          setCurrentStatus(fallback.status);
  ...
  if (!ret) return <div className="admin-empty"><div className="admin-empty-title">Return not found</div><a href="/admin/returns" className="admin-btn admin-btn--primary">Back to Returns</a></div>;
